...
- **Data**: PostgreSQL 15 + PostGIS for spatial/region-aware rules; keep SQLite only for local dev. Connection URLs must support read replicas in KSA regions.
...
4. **Observability & Launch (Week 8)**: Prometheus/Grafana dashboards, Storybook alignment to BrainSAIT tokens, performance tuning to hit <500ms search/<2s API, UAT with 50 KSA clients.

## Success Criteria
- 99.9% SBS mapping accuracy; Arabic code search p99 <500ms; API p99 <2s across regions.
